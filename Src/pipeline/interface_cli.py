"""
FSY Pipeline CLI Interface

Main orchestrator for the FSY pipeline. Coordinates all modules in sequence
...
def parse_arguments() -> Tuple[float, float, int, int, Dict[str, str], str, str, str, bool | None]:
    """
    Parse command-line arguments for the FSY pipeline.
    
    Returns:
        Tuple of (lon, lat, start_year, end_year, netcdf_files, scenario, gcm, rcm, legacy_ftmy_copy)
        where netcdf_files is a dictionary with keys:
        tas, tasmax, tasmin, rsds, sfcWind, pr, potevap
    """
    parser = argparse.ArgumentParser(
        description="FSY Pipeline - Generate synthetic weather files from climate projections"
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help="Also write legacy FTMY_*.epw copy (CLI overrides env var if set)"
    )
    ...
    gcm = args.gcm
    rcm = args.rcm
    legacy_ftmy_copy = args.legacy_ftmy_copy
    ...
    return lon, lat, start_year, end_year, netcdf_files, scenario, gcm, rcm, legacy_ftmy_copy
...
def main():
    ...
    Orchestrates the complete FSY pipeline:
    ...
    """
    print("=" * 80)
    print("FSY PIPELINE - CLEAN REBUILD")
    print("=" * 80)
    print()
    
    # Step 1: Parse arguments
    lon, lat, start_year, end_year, netcdf_files, scenario, gcm, rcm, legacy_ftmy_copy_cli = parse_arguments()
    ...
    legacy_ftmy_copy = (
        PipelineConfig.LEGACY_FTMY_OUTPUT_COPY
        if legacy_ftmy_copy_cli is None else legacy_ftmy_copy_cli
    )
    print(f"\nResults will be saved to: {results_path}")
    print(f"Legacy FTMY output copy enabled: {legacy_ftmy_copy}")
    print()
    ...
    try:
        ...
        epw_file = generate_epw_file(
            ...
            legacy_ftmy_copy=legacy_ftmy_copy
        )
    ...
    print("  - FSY_*.epw")
    if legacy_ftmy_copy:
        print("  - FTMY_*.epw (legacy compatibility copy)")
    print()
